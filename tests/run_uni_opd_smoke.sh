...
if [[ $(python -c 'import torch; print(torch.cuda.device_count())') -ne 8 ]]; then
  echo "Uni-OPD smoke tests require exactly 8 visible GPUs." >&2
  exit 1
fi

configs=(
  uni_opd_qwen3_8b_to_1p7b_no_thinking_smoke_2steps
  uni_opd_qwen3_8b_to_1p7b_thinking_smoke_2steps
)

mkdir -p "$project_root/outputs/uni_opd_smoke_logs" "$WANDB_DIR"

run_smoke() {
  local config_name=$1
  local run_name="temp_${config_name}"
  local log_file="$project_root/outputs/uni_opd_smoke_logs/${config_name}.log"
  ...
  grep -q "uni-opd/train/policy_loss" "$log_file"
...
echo "All Uni-OPD GPU smoke tests passed."
