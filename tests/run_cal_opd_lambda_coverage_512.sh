...
# Collect one matched 512-question rollout batch for each Qwen3 thinking mode.
...
configs=(
  cal_opd_qwen3_4b_to_1p7b_no_thinking_lambda_coverage_512
  cal_opd_qwen3_4b_to_1p7b_thinking_lambda_coverage_512
)
...
for config_name in "${configs[@]}"; do
  ...
  tensors_path="$output_dir/cal_opd_token_tensors.pt"

  if [[ -f "$stats_path" ]] && [[ $(wc -l < "$stats_path") -eq 1 ]] && [[ -f "$tensors_path" ]]; then
  ...
  if [[ ! -f "$stats_path" ]] || [[ $(wc -l < "$stats_path") -ne 1 ]] || [[ ! -f "$tensors_path" ]]; then
    echo "Expected one statistics row and token tensors under: $output_dir" >&2
    exit 1
  fi
