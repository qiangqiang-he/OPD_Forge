...
EXPECTED_CONFIG_MODELS = {
    "pub_correct_rl_qwen3_0p6b_no_thinking_cliphigh0p27_lr1e-6_len10k_1000steps": "Qwen3-0.6B",
    "pub_correct_rl_qwen3_1p7b_no_thinking_cliphigh0p27_lr1e-6_len10k_1000steps": "Qwen3-1.7B",
    "pub_correct_rl_qwen3_1p7b_repro_no_thinking_cliphigh0p27_lr1e-6_len10k_1000steps": "Qwen3-1.7B",
    "pub_correct_rl_qwen3_4b_no_thinking_cliphigh0p27_lr1e-6_len10k_1000steps": "Qwen3-4B",
    "pub_correct_rl_qwen3_8b_no_thinking_cliphigh0p27_lr1e-6_len10k_1000steps": "Qwen3-8B",
}
...
def test_correct_rl_no_thinking_publication_contract(config_name):
    ...
    assert "advpt" not in config_name.lower()
    assert "gamma" not in config_name.lower()
    assert "cliphigh0p27_lr1e-6_len10k_1000steps" in config_name
    ...
    assert "correct_rl_gamma" not in config.algorithm
    ...
    assert float(actor.optim.clip_grad) == pytest.approx(0.5)
